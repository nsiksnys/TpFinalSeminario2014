...
public class ReservaController {
	...
	public String alta(@ModelAttribute ReservaForm formulario, Principal principal, HttpServletRequest request, RedirectAttributes ra) 
	{
		...
		//int limiteReserva = logicaNegocio.cantidadTotalReservas(item);
		
//		if(limiteReserva > 7){
//			MessageHelper.addErrorAttribute(ra, "El cliente supero el total de reservas para esta funcion.");//agrego el mensaje de error
//			return "redirect:/reserva/alta";
//		}

		if (logicaNegocio.isLimiteReservasHechasSuperado(item))
		{
			MessageHelper.addErrorAttribute(ra, "El cliente supero el total de reservas para esta funcion.");//agrego el mensaje de error
			return "redirect:/reserva/alta";
		}
		if (logicaNegocio.isLimiteReservasPorSuperar(item, Integer.parseInt(formulario.getCantidad()))){
			MessageHelper.addErrorAttribute(ra, "Por favor revise el formulario no puedo reservar mas de 7 asientos.");//agrego el mensaje de error
			return "redirect:/reserva/alta";
		}
//		if (Integer.parseInt(formulario.getCantidad()) + limiteReserva > 7){ // SI ES MAYOR A 7
//			MessageHelper.addErrorAttribute(ra, "Por favor revise el formulario no puedo reservar mas de 7 asientos.");//agrego el mensaje de error
//			return "redirect:/reserva/alta";
//		}
		//=========================
		...
		request.getSession().setAttribute("fecha", new SimpleDateFormat("yyyy-MM-dd").format(item.getFechaReserva()));
	...
	public @ResponseBody boolean isAsientoDisponible(@RequestParam Long asiento, @RequestParam Long funcion,@RequestParam String fecha, Principal principal)
	{
		//LOG.info("/reserva/getAsientoStatus: pedidas status del asiento " + asiento + " para la funcion " + funcion);
		return logicaNegocio.isAsientoDisponible(asiento, funcion,fecha);
	}
